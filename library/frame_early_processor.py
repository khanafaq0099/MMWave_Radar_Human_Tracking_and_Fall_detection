...
from library.utils import np_filter


class FrameEProcessor:  # early processing for frame of each radar before merging
    ...
    def FEP_boundary_filter(self, data_points):
        ...
        data_points, _ = np_filter(data_points, idx=0, range_lim=self.xlim)
        data_points, _ = np_filter(data_points, idx=1, range_lim=self.ylim)
        data_points, _ = np_filter(data_points, idx=2, range_lim=self.zlim)
        return data_points

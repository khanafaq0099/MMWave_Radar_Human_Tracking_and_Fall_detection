...
MANSAVE_PERIOD = 60  # second, the time period saved for manual save
# auto save
...
RADAR_CFG_LIST = [
    {'name'          : 'IWR1843_Ori',
     ...
     'SNRV_threshold'  : {'range': (0, None), 'speed_none_0_exception': False},  # if speed_none_0_exception is True, then the data with low SNR but with speed will be reserved
     },
    {'name'          : 'IWR1843_Side',
     'cfg_port_name' : 'COM4',
     'data_port_name': 'COM3',
     ...
     'ylim'          : (0.25, 4),
     'zlim'          : None,
     'pos_offset'    : (-0.15, 0.2, 2.5),  # default pos_offset is (0, 0, 0)
     'facing_angle'  : {'angle': (-60, 0, 0), 'sequence': None},  # right-hand global coord-sys, (x, y, z): [-180, 180] positive counted anti-clockwise when facing from axis end towards origin, default rotation sequence: zyx
     'SNRV_threshold'  : {'range': (0, None), 'speed_none_0_exception': False},  # if speed_none_0_exception is True, then the data with low SNR but with speed will be reserved
     },
    {'name'          : 'IWR1843_Top',
     'cfg_port_name' : 'COM7',
     ...
     'ylim'          : (0.25, 4),
     'zlim'          : None,
     'pos_offset'    : (-0.15, 1.7, 2.5),  # default pos_offset is (0, 0, 0)
...
FRAME_POST_PROCESSOR_CFG = {  # post process config
    ...
    'FPP_global_xlim' : (-1, 1),  # the x-direction limit for merged cloud points from all radars, set as [a, b), from global view
...
DBSCAN_GENERATOR_CFG = {  # DBSCAN para config
    ...
    'Dynamic_SNR_0_above'  : {  # for data points with energy above 0
        'DBS_eps'        : 0.2,
        'DBS_min_samples': 15,
    },
    'Dynamic_SNR_100_above': {
        'DBS_eps'        : 0.3,
        'DBS_min_samples': 12,
    },
    'Dynamic_SNR_200_above': {
        'DBS_eps'        : 0.5,
        'DBS_min_samples': 6,
    },
    'Dynamic_SNR_300_above': {
        'DBS_eps'        : 0.8,
        'DBS_min_samples': 3,
...
BGNOISE_FILTER_CFG = {  # Background noise filter config
    ...
    'BGN_accept_SNR_threshold': (None, 200),  # the noise with this SNR range will be accepted when BGN update, it is for DBS noise
    'BGN_filter_SNR_threshold': (None, 200),  # the noise with this SNR range will be filtered when BGN filter
...
HUMAN_OBJECT_CFG = {  # human object config for each object bin
    ...
    'sub_possibility_proportion': (1.8, 1.8, 2, 0.5),  # the coefficient for the possibility proportion
